...
export default function startServer(port) {
  ...
  app.get("/api/getwhiteboard", function (req, res) {
  ...
  app.get("/api/getWid", function (req, res) {
    let query = escapeAllContentStrings(req["query"]);
    const wid = query["wid"];
    const at = query["at"];
    if (accessToken === "" || accessToken == at) {
      ...
      if (imagedata && imagedata != "") {
        ...
        fs.writeFile(savingPath, imagedata, "base64", function (err) {
          if (err) {
            ...
            if (webdavaccess) {
              if (enableWebdav) {
                saveImg2Webdav(savingPath, filename, webdavaccess, function (err) {
                  if (err) {
                    console.log("error", err);
                    callback(err);
                  } else {
  ...
  function saveImg2Webdav(imagepath, filename, webdavaccess, callback) {
    if (webdavaccess) {
