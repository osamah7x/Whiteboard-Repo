...
import allKeybinds from "./keybinds.js";
...
function initWhiteboard() {
  $(document).ready(function () {
    ...
    whiteboard.getWhiteboard("#whiteboardContainer", {
    ...
    $.get(subdir + "/api/getwhiteboard", { wid: whiteboardId, at: accessToken }).done(function (
      data
    ) {
      whiteboard.loadData(data);
      if (copyfromwid && data.length == 0) {
        $.get(subdir + "/api/getwhiteboard", {
          wid: copyfromwid,
          at: accessToken,
        }).done(function (data) {
          whiteboard.loadData(data);
    ...
    Object.entries(allKeybinds).forEach(([key, functionName]) => {
